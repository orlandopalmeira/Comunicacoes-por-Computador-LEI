...
class DataBase:
    ...
    def existsDomain2(self,name: str):
        res = False
        name_ = self.cname[name][0] if name in self.cname else name
        if self.valid:
            for x in self.ns:
        ...
        try: 
            ...
            for line in lines:
                if line[1] == 'DEFAULT':
                    ...
                    if len(line) == 4:
                        ptr_ip_addr = DataBase.__replace_defaults_and_complete(line[0],macros)
    ...
    def __getExtraValues(self, response_values, authorities_values):
        ...
        A: nome_servidor -> lista((endereço_ip,TTL,prioridade))
           server_name A endereço_ip TTL prioridade
        """
        ...
        for serv_name in self.a:
            for (ip_add,ttl,prio) in self.a[serv_name]:
                if (serv_name in rv_values) or (serv_name in av_values):
                    p = '' if prio == -1 else f' {prio}'
                    result.append(f'{serv_name} A {ip_add} {ttl}{p}')
        return result

    def getQueryResponse(self, name: str, type_of_value: str):
        if self.responseCode1(name,self.int_from_type_of_value[type_of_value]):
            ...
            extra_values = self.__getExtraValues(response_values=response_values,authorities_values=authorities_values)
            return (response_values,authorities_values,extra_values)

    def __getAuthoritiesValues2(self, name: str):
        '''
        Implementa a recolha dos RESPONSE VALUES quando o response code é 2
        '''
        name_ = self.cname[name][0] if name in self.cname else name
        res = []
        for dom in self.ns:
            if dom in name_:
                for (serv_n,ttl,prio) in self.ns[dom]:
                    p = '' if prio == -1 else f' {prio}'
                    res.append(f'{name_} NS {serv_n} {ttl}{p}')

        return res
    
    def __getExtraValues2(self, response_values = [], authorities_values = []):
        '''
        Implementa a recolha dos EXTRA VALUES quando não há resposta direta a query
        '''
        rv_values = set(map(lambda s: s.split(' ')[2],response_values)) # vai buscar o campo value
        av_values = set(map(lambda s: s.split(' ')[2],authorities_values)) # vai buscar o campo value 
        result = []
    ...
    def getQueryResponse2(self, name: str):
        authorities_values = self.__getAuthoritiesValues2(name)
        extra_values = self.__getExtraValues2(name)
        return (authorities_values,extra_values)
