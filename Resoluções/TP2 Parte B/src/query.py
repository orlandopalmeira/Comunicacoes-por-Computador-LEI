...
class Query:
    ...
    def encode(self) -> bytes:
        ...
        result += len(self.name).to_bytes(1,'big')
        result += self.name.encode('utf-8') # TODO arranjar maneira de converter em bytes
        result += self.type_of_value.to_bytes(1,'big')
        result += len(str(self.response_values)).to_bytes(2,'big')
        result += str(self.response_values).encode('utf-8') # TODO arranjar maneira de converter em bytes
        result += len(str(self.authorities_values)).to_bytes(2,'big')
        result += str(self.authorities_values).encode('utf-8') # TODO arranjar maneira de converter em bytes
        result += len(str(self.extra_values)).to_bytes(2,'big')
        result += str(self.extra_values).encode('utf-8') # TODO arranjar maneira de converter em bytes
        return result
    
    @staticmethod
    def decode(serialized: bytes):
        """
        Descodifica bytes e converte-os numa query.
        """
        b = serialized
        message_id = int.from_bytes(b[0:2],'big')
        b = b[2:]
        flags = int.from_bytes(b[0:1],'big')
        b = b[1:]
        response_code = int.from_bytes(b[0:1],'big')
        b = b[1:]
        number_of_values = int.from_bytes(b[0:1],'big')
        b = b[1:]
        number_of_authorities = int.from_bytes(b[0:1],'big')
        b = b[1:]
        number_of_extra_values = int.from_bytes(b[0:1],'big')
        b = b[1:]
        len_name = int.from_bytes(b[0:1],'big')
        b = b[1:]
        name = b[0:len_name].decode('utf-8')
        b = b[len_name:]
        type_of_value = int.from_bytes(b[0:1],'big')
        b = b[1:]
        len_resp_values = int.from_bytes(b[0:2],'big')
        b = b[2:]
        resp_values = literal_eval(b[0:len_resp_values].decode('utf-8'))
        b = b[len_resp_values:]
        len_auth_values = int.from_bytes(b[0:2],'big')
        b = b[2:]
        auth_values = literal_eval(b[0:len_auth_values].decode('utf-8'))
        b = b[len_auth_values:]
        len_ext_values = int.from_bytes(b[0:2],'big')
        b = b[2:]
        ext_values = literal_eval(b[0:len_ext_values].decode('utf-8'))
        return Query(message_id,flags,response_code,number_of_values,number_of_authorities,number_of_extra_values,name,type_of_value,resp_values,auth_values,ext_values)
